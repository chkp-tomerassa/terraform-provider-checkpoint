...
func testAccCheckpointManagementServiceSctpDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Primary.ID != "" {
			res, _ := client.ApiCall("show-service-sctp", map[string]interface{}{"uid": rs.Primary.ID}, client.GetSessionID(), true, client.IsProxyUsed())
			if res.Success {
...
func testAccCheckCheckpointManagementServiceSctpExists(resourceTfName string, res *map[string]interface{}) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		response, err := client.ApiCall("show-service-sctp", map[string]interface{}{"uid": rs.Primary.ID}, client.GetSessionID(), true, client.IsProxyUsed())
		if !response.Success {
			return err
		}
