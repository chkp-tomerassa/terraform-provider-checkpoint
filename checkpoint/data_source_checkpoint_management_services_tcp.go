...
func dataSourceManagementServicesTcp() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceManagementServicesTcpRead,
...
func dataSourceManagementServicesTcpRead(d *schema.ResourceData, m interface{}) error {

	client := m.(*checkpoint.ApiClient)

	showServicesTcpRes := checkpoint.APIResponse{}
	var err error
	fetchAll, _ := d.GetOkExists("fetch_all")

	if fetchAll.(bool) {
		showServicesTcpRes, err = client.ApiQuery("show-services-tcp", "full", "objects", true, map[string]interface{}{})
	} else {
		...
		showServicesTcpRes, err = client.ApiCallSimple("show-services-tcp", payload)
	}
	...
	log.Println("Read ServicesTcp - Show JSON = ", servicesTcp)

	d.SetId("show-services-tcp-" + acctest.RandString(10))

	if v := servicesTcp["from"]; v != nil {
		_ = d.Set("from", v)
	}

	if v := servicesTcp["to"]; v != nil {
		_ = d.Set("to", v)
	}

	if v := servicesTcp["total"]; v != nil {
		_ = d.Set("total", v)
	}

	if v := servicesTcp["objects"]; v != nil {
		...
		if len(objectsList) > 0 {
			...
			for i := range objectsList {
				...
				if v := objectMap["use-delayed-sync"]; v != nil {
					objectMapToAdd["use_delayed_sync"] = v
				}

				if v := objectMap["delayed-sync-value"]; v != nil {
					objectMapToAdd["delayed_sync_value"] = v
				}
