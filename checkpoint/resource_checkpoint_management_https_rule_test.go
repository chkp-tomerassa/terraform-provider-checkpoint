...
func testAccCheckpointManagementHttpsRuleDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Primary.ID != "" {
			res, _ := client.ApiCall("show-https-rule", map[string]interface{}{"uid": rs.Primary.ID, "layer": "New Layer 2"}, client.GetSessionID(), true, client.IsProxyUsed())
			if res.Success {
...
func testAccCheckCheckpointManagementHttpsRuleExists(resourceTfName string, res *map[string]interface{}) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		response, err := client.ApiCall("show-https-rule", map[string]interface{}{"uid": rs.Primary.ID, "layer": "Default Layer"}, client.GetSessionID(), true, client.IsProxyUsed())
		if !response.Success {
			return err
		}
